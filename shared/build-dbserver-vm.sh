...
echo "CREATE DATABASE snaker;" | mysql
...
# the "snaker" database that we just created, above.
echo "GRANT ALL PRIVILEGES ON snaker.* TO 'webuser'@'%'" | mysql
...
# the user to connect as (webuser) and the database to use (snaker).
cat /vagrant/setup-database.sql | mysql -u webuser snaker
